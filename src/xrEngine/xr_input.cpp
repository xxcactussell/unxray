...
CInput::CInput(const bool exclusive)
{
    ...
    if (SDL_JoystickID* joysticks = SDL_GetGamepads(&num_joysticks)) {
        for (int i = 0; i < num_joysticks; ++i)
            OpenController((int)joysticks[i]);
        SDL_free(joysticks);
    }
}
...
{
    if (!SDL_IsGamepad((SDL_JoystickID)idx))
        return;

    const auto controller = SDL_OpenGamepad((SDL_JoystickID)idx);
    if (!controller)
        return;
...
void CInput::MouseUpdate()
{
    ...
    for (int i = 0; i < count; ++i)
    {
        ...
        case SDL_EVENT_MOUSE_MOTION:
            mouseMoved = true;
            offs[0] += (int)event.motion.xrel;
            offs[1] += (int)event.motion.yrel;
            mouseAxisState[0] = (int)event.motion.x;
            mouseAxisState[1] = (int)event.motion.y;
            break;
        ...
        case SDL_EVENT_MOUSE_WHEEL:
            ...
            mouseAxisState[2] += (int)event.wheel.x;
            mouseAxisState[3] += (int)event.wheel.y;
            break;
        }
    }
...
bool KbdKeyToButtonName(const int dik, xr_string& result)
{
    ...
    if (dik >= 0)
    {
        cpcstr name = SDL_GetKeyName(SDL_GetKeyFromScancode((SDL_Scancode)dik, SDL_KMOD_NONE, true));
        if (name && name[0])
        {
...
bool CInput::iSetMousePos(const Ivector2& p, bool global /*= false*/) const
{
    ...
    {
#if SDL_HAS_CAPTURE_AND_GLOBAL_MOUSE
        SDL_WarpMouseGlobal((float)p.x, (float)p.y);
        return true;
#endif
        // if SDL_HAS_CAPTURE_AND_GLOBAL_MOUSE unavailable
    ...
    SDL_WarpMouseInWindow(Device.m_sdlWnd, (float)p.x, (float)p.y);
    return !global;
}
...
void CInput::GrabInput(const bool grab)
{
    ...
    SDL_SetWindowMouseGrab(Device.m_sdlWnd, grab ? true : false);

    // Grab the mouse
    if (exclusiveInput)
        SDL_SetWindowRelativeMouseMode(Device.m_sdlWnd, grab ? true : false);
...
void CInput::EnableTextInput()
{
    ...
    if (textInputCounter == 1)
        if (Device.m_sdlWnd) SDL_StartTextInput(Device.m_sdlWnd);
...
void CInput::DisableTextInput()
{
    ...
    if (textInputCounter == 0)
        if (Device.m_sdlWnd) SDL_StopTextInput(Device.m_sdlWnd);
