...
SRagdollSettings CActiveRagdollSettingsBuilder::BuildSettings(IKinematics* kinematics, const CActiveRagdollSkeletonMapper& mapper) {
    ...
    for (u32 part_idx = 0; part_idx < num_parts; ++part_idx) {
        ...
        u16 depth = 0;
        u16 total_bones = kinematics->LL_BoneCount();
        while (parent_bone_id != u16(-1) && parent_bone_id != BI_NONE && parent_bone_id != bone_id && depth++ < total_bones) {
            u16 parent_part = mapper.BoneToPart(parent_bone_id);
            if (parent_part != u16(-1)) {
                part_desc.parent_index = (int)parent_part;
                break;
            }
            u16 next_parent = kinematics->GetBoneData(parent_bone_id).GetParentID();
            if (next_parent == parent_bone_id) break;
            parent_bone_id = next_parent;
        }
        
        if (part_idx > 0 && part_desc.parent_index == -1) {
            part_desc.parent_index = 0;
        }
        ...
        if (is_pelvis) {
            ...
            // Intermediate bone without explicit shape
            PhysicsShapeHandle helper_sphere = GetPhysicsCore()->CreateSphereShape(0.12f);
            part_desc.shape = GetPhysicsCore()->CreateRotatedTranslatedShape(helper_sphere, Fvector().set(0, 0, 0), Fquaternion().identity());
            part_desc.mass = std::max(bone_data.get_mass(), 10.0f);
        }
    }
    
    // Bottom-up mass propagation: parents must always be at least as heavy as their children
    for (int i = (int)num_parts - 1; i >= 0; --i) {
        int p_idx = settings.parts[i].parent_index;
        if (p_idx >= 0 && p_idx < (int)num_parts && p_idx != i) {
            settings.parts[p_idx].mass = std::max(settings.parts[p_idx].mass, settings.parts[i].mass);
        }
    }
...
void CActiveRagdollController::KnockDown(u16 bone_id, const Fvector& dir, float impulse, const Fvector& hit_pos) {
    ...
    if (impulse >= 0.5f) {
        ...
        float dir_mag = blast_dir.magnitude();
        if (dir_mag > 0.001f) {
            blast_dir.mul(1.0f / dir_mag);
            float clamped_impulse = std::clamp(impulse * 0.65f, 0.0f, 2500.0f);
            u32 part_count = (u32)m_simulated_matrices.size();
            if (part_count > 0) {
                // Pelvis (root) gets primary momentum (90%)
                Fvector root_imp = blast_dir;
                root_imp.mul(clamped_impulse * 0.90f);
                GetPhysicsCore()->ApplyRagdollLinearImpulse(m_ragdoll_handle, 0, root_imp);

                // Limbs get cohesive impulse (15%)
                float per_part_mag = clamped_impulse * 0.15f;
        ...
        if (dir_mag > 0.001f) {
            ...
            if (part_count > 0) {
                ...
                if (part_idx != 0 && part_idx < part_count) {
                    Fvector local_hit = blast_dir;
                    local_hit.mul(clamped_impulse * 0.15f);
...
void CActiveRagdollController::ApplyLinearImpulse(const Fvector& dir, float impulse) {
    ...
    Fvector norm_dir = dir;
    norm_dir.normalize_safe();
    if (norm_dir.magnitude() < 0.001f) return;

    // Apply primary impulse to root (pelvis) (90%)
    Fvector root_imp = norm_dir;
    root_imp.mul(impulse * 0.90f);
    GetPhysicsCore()->ApplyRagdollLinearImpulse(m_ragdoll_handle, 0, root_imp);

    // Apply cohesive impulse to limbs (35%)
    Fvector part_imp = norm_dir;
    part_imp.mul(impulse * 0.35f);
    for (u32 i = 1; i < part_count; ++i) {
        GetPhysicsCore()->ApplyRagdollLinearImpulse(m_ragdoll_handle, i, part_imp);
    }
}
