...
static void cammera_shell_collide_callback_common(
    bool& do_collide, bool bo1, float depth, CPhysicsGeom* my_geom, CPhysicsGeom* oposite_geom, u16 material_1, u16 material_2)
{
    if (!do_collide)
        return;
    ...
    // В Jolt Physics флаги материалов обрабатываются через слои/группы, 
    // но если нужно ручное исключение (как SGameMtl::flPassable), 
    // это должно проверяться через GMLib по индексам material_1 / material_2.

    if (!my_geom || !oposite_geom) 
        return;

    // Получаем привязанные игровые объекты (если метод PhysicsRefObject был добавлен в CPhysicsGeom, 
    // иначе можно достать через get_callback_data())
    IPhysicsShellHolder* my_ref = (IPhysicsShellHolder*)my_geom->get_callback_data();
    IPhysicsShellHolder* oposite_ref = (IPhysicsShellHolder*)oposite_geom->get_callback_data();

    if (oposite_ref && oposite_ref == my_ref)
        return;
    if (oposite_ref && !oposite_ref->IsCollideWithActorCamera()) 
        return;
        
    if (depth > camera_collision_sckin_depth / 2.f)
        cam_collided = true;

    if (!cam_step)
        return;

    // Разрешаем Jolt Physics самостоятельно растолкать тела
    do_collide = true;
}

static void cammera_shell_collide_callback(
    bool& do_collide, bool bo1, 
    CPhysicsGeom* my_geom, CPhysicsGeom* oposite_geom, 
    const Fvector& contact_normal, const Fvector& contact_pos, 
    SGameMtl* material_1, SGameMtl* material_2)
{
    cammera_shell_collide_callback_common(do_collide, bo1, 0.0f, my_geom, oposite_geom, 0, 0);
}
...
{
    do_collide = false;
    if (!oposite_geom) return;

    CPHObject* oposite_ph_obj = (CPHObject*)oposite_geom->get_callback_data();
    
    if (!oposite_ph_obj || oposite_ph_obj->CastType() != CPHObject::tpCharacter)
        return;

    IPhysicsShellHolder* oposite_ref = (IPhysicsShellHolder*)oposite_geom->get_callback_data();
    if (!oposite_ref || !(oposite_ref->IsStalker()))
        return;
...
CPhysicsShell* create_camera_shell(IPhysicsShellHolder* actor)
{
    ...
    CPhysicsGeom* character_test_geom = smart_cast<CPhysicsGeom*>(xr_new<CCylinderGeom>(cyl));
    ...
    // В новой архитектуре массы устанавливаются скалярно
    shell->setEquelInertiaForEls(actor_camera_hudge_mass);
    
    VERIFY(roote);
    roote->set_local_mass_center(Fvector().set(0, 0, 0));
    VERIFY(roote->numberOfGeoms());
    
    CPhysicsGeom* root_geom = roote->geometry(0);
    VERIFY(root_geom);
...
void do_collide_and_move(const Fmatrix& xform, IPhysicsShellHolder* l_actor, CPhysicsShell* shell, CPhysicsElement* roote)
{
    ...
    if (cam_collided)
    {
        ...
        for (u16 i = 0; i < cam_correction_steps_num; ++i)
        {
            ...
            // В Jolt PureStep отработает разрешение коллизий (выталкивание)
            shell->PureStep(); 
            if (!cam_collided)
                break;
        }
...
bool test_camera_collide(CCameraBase& camera, float _viewport_near, IPhysicsShellHolder* l_actor, Fvector& vPosOffset, float fBoxSizeMod)
{
    Fvector box_size;
    Fmatrix xform;
