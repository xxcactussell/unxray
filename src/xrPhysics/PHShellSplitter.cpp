...
#include "xrPhysicsCore/IPhysicsCore.h" // Подключаем абстрактное ядро
    ...
    const CShellSplitInfo& spl_inf, CPHShell* dest, u16 jt_add_shift, u16 el_add_shift)
{
    ...
    for (; i_elem != e_elem; ++i_elem)
    {
        ...
        for (; f_i != f_e; ++f_i)
        {
            f_i->m_end_el_num -= passed_shift_e;
            f_i->m_start_el_num -= passed_shift_e;
            f_i->m_end_jt_num -= passed_shift_j;
            f_i->m_start_jt_num -= passed_shift_j;
        }
    }
    ...
    if (spl_inf.m_end_jt_num != u16(-1))
    {
        ...
        for (; i_joint != e_joint; ++i_joint)
        {
            ...
            jointDestroyInfo->m_end_element -= passed_shift_e;
            jointDestroyInfo->m_end_joint -= passed_shift_j;
        }
    }
    ...
    for (; i_elem != e_elem; ++i_elem)
    {
        ...
        for (; f_i != f_e; ++f_i)
        {
            f_i->m_end_el_num -= shift_e;
            f_i->m_start_el_num -= shift_e;
            f_i->m_end_jt_num -= shift_j;
            f_i->m_start_jt_num -= shift_j;
        }
    }
    ...
    for (; i_joint != e_joint; ++i_joint)
    {
        ...
        if (jointDestroyInfo->m_end_element > spl_inf.m_end_el_num)
            jointDestroyInfo->m_end_element -= shift_e;
        if (jointDestroyInfo->m_end_joint > spl_inf.m_end_jt_num)
            jointDestroyInfo->m_end_joint -= shift_j;
    }
    ...
    for (; spl_i != spl_e; ++spl_i)
    {
        ...
        {
            if (spl_i->m_element != u16(-1) && spl_i->m_element >= spl_inf.m_start_el_num) break;
        }
        else
        {
            if (spl_i->m_joint != u16(-1) && spl_i->m_joint >= spl_inf.m_start_jt_num) break;
        }
        if (spl_i->m_element != u16(-1) && spl_i->m_element > spl_inf.m_end_el_num) spl_i->m_element -= shift_e;
        if (spl_i->m_joint != u16(-1) && spl_i->m_joint > spl_inf.m_end_jt_num) spl_i->m_joint -= shift_j;
    }
    ...
    for (; spl_i != spl_e; ++spl_i)
    {
        if (spl_i->m_type == CPHShellSplitter::splElement)
        {
            if (spl_i->m_element != u16(-1) && spl_i->m_element >= spl_inf.m_end_el_num) break;
        }
        else
        {
            if (spl_i->m_joint != u16(-1) && spl_i->m_joint >= spl_inf.m_end_jt_num) break;
        }
        if (spl_i->m_element != u16(-1)) spl_i->m_element -= passed_shift_e;
        if (spl_i->m_joint != u16(-1)) spl_i->m_joint -= passed_shift_j;
    }

    auto i_to = spl_i;
    for (; spl_i != spl_e; ++spl_i)
    {
        if (spl_i->m_element != u16(-1)) spl_i->m_element -= shift_e;
        if (spl_i->m_joint != u16(-1)) spl_i->m_joint -= shift_j;
    }
...
shell_root CPHShellSplitterHolder::ElementSingleSplit(const element_fracture& split_elem, const CPHElement* source_element)
{
    ...
    if ((split_elem.first)->FracturesHolder()) 
...
void CPHShellSplitterHolder::SplitElement(u16 aspl, PHSHELL_PAIR_VECTOR& out_shels)
{
    ...
    if (!E->FracturesHolder())
        m_splitters.erase(spl_i); 
    else
        spl_i->m_breaked = false; 
}
